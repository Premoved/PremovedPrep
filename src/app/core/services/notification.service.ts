...
/** Opened in a new tab, so whatever is on this page survives. */
...
export class NotificationService {
	...
	private push(kind: NoticeKind, message: string, timeoutMs: number): void {
		...
		// Attached here, not at each call site: every 401 in the app arrives as this one string.
		const action = text === SIGN_IN_REQUIRED ? SIGN_IN : undefined;
		this.items.update((list) => [...list, { id, kind, message: text, action }]);

		// No NgZone: the app is zoneless, so the signal write is what schedules the re-render.
